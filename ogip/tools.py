...
def convolve(source_model: Callable, rmf: RMF, arf: Optional[ARF]=None, args=None):
    ...
    r = (np.outer(_arf * source_model(rmf.c_energ, *args) * rmf.d_energ, np.ones_like(rmf._e_min))*rmf._matrix).sum(0)
...
def fit(source_model_generator: Callable, p0: list, spectra: list[tuple[PHAI, RMF, Optional[ARF], np.ndarray]], method='COBYLA'):
    def F(p):
        r = 0

        for _ in spectra:
            r += get_mloglike(_[0], source_model_generator(p), rmf=_[1], arf=_[2], mask=_[3])
    ...
    logger.debug("optimized model spec %s", convolve(source_model_generator(r.x), spectra[0][1], spectra[0][2]))
...
def sample(source_model_with_pars: Callable, p0: list[tuple[float, float, float]], 
           spectra: list[tuple[PHAI, RMF, Optional[ARF], np.ndarray]],
           nwalkers=100, nsteps=10,
           n_processes=None,
           ):
    ...
    def S(pool=None):
        sampler = emcee.EnsembleSampler(nwalkers, len(p0), log_prob, pool=pool, args=(p0, spectra, source_model_with_pars))
...
def plot(pha: PHAI, model: Callable, rmf: RMF, arf: Optional[ARF]=None, 
         ...
         scale_factor=1, plot_model=False):
    ...
    if plot_model:
        if unfolded:        
            plt.plot(rmf.c_energ, model(rmf.c_energ) * rmf.c_energ**e_power * scale_factor)
        else:
            ...
            plt.step(
                e1,
                model_spec/rmf.d_e * scale_factor,
                where='post',
                **plot_kwargs
            )
    ...
    if pha is not None:
        ...
        rate_or_uplim = pha._rate/rmf.d_e * unfolding_factor * rmf.c_e**e_power * scale_factor
        err = pha._stat_err/rmf.d_e * unfolding_factor * rmf.c_e**e_power * scale_factor
